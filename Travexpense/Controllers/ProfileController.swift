...
class ProfileViewController: UITableViewController {
  ...
  override func viewDidLoad() {
    ...
    if let image = ImageHelper.shared.image(forKey: photoURL.absoluteString as NSString) {
      profileImageButton.setImage(image, for: .normal)
    } else {
      ImageHelper.shared.fetchImage(urlString: photoURL.absoluteString) { (appError, image) in
